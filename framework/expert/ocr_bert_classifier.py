...
class OCRBertClassifier:
    ...
    def generate_bert_dataset(self, dataset_path, data_type="train"):
        ...
        for label_index, label in enumerate(sorted(os.listdir(dataset_path))):
            ...
            for img in tqdm(os.listdir(label_dir)):
                ...
                # need vocab length > 1
                orig_str ="".join([remove_special_chars(ret[1]) for ret in bounds])
                # need vocab length > 1
                ...
                if len(orig_str) < 1:
                    ...
                    for vocab in jieba.cut(orig_str, cut_all=True):
                        if len(vocab) > 1:
                            x_array.append(orig_str)
                            y_array.append(label_index)
    ...
    def fit(self, train_dataset, val_dataset, test_dataset, epochs=5, batch_size=32, lr=2e-5):
        train_dataset = self.generate_bert_dataset(train_dataset, "train")
        val_dataset = self.generate_bert_dataset(val_dataset, "val")
        test_dataset = self.generate_bert_dataset(test_dataset, "test")
    ...
    def ensemble_eval(self, test_dataset,bert_model_path='/home/kaiyuan.xu/pycharm/framework/expert/saved_model-skip-empty',
        ...
        test_bert_data = self.generate_bert_dataset(test_dataset, "test")
        assert test_bert_data.data_size>0
        ...
        acc_count += (cls_model_result["class"] == test_bert_data.empty_label).value_counts()[0]
        ...
        y_label = test_bert_data.y.numpy()
        acc_count += numpy.sum((y_label == y_pred) != 0)
        accuary = acc_count / test_bert_data.data_size
        print(accuary)




def append_single_ocr():
    for label_index, label in enumerate(sorted(os.listdir(dataset_path))):
        label_map[label] = label_index
        label_dir = os.path.join(dataset_path, label)
        for img in tqdm(os.listdir(label_dir)):
