...
def url_data(url, path=None, overwrite=False, overwrite_folder=False, sha1_hash=None, root=None, disp_depth=1):
    ...
    path : str, optional
        ...
        ~/.autotorch directory with same name as in url.
        You can also change the default behavior by editing ~/.autotorch/config.yaml.
    ...
    sha1_hash : str, optional
        ...
        but doesn't match.
    root : str, optional
        Destination root dir to store extracted files. By default it's default in ~/.autotorch directory.
...
class Config:
    "Setup config at `~/.autotorch` unless it exists already."
    config_path = Path(os.getenv('MXNET_HOME', '~/.autotorch')).expanduser()
    ...
    def save_file(self, config):
        "save config file at default config location `~/.autotorch/config.yml`."
