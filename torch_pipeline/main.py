...
def parse_args():
    ...
    parser.add_argument('--rmsprop-eps', default=1e-8, type=float,
                        help="value of eps parameter in rmsprop optimizer (default: 1e-8)", )
    parser.add_argument('--adamw-eps', default=1e-8, type=float,
    ...
    if args.lr_schedule == "step":
        if args.auto_step:
            ...
            lr_policy = StepLRScheduler(
                optimizer=optimizer, base_lr=args.lr, steps=auto_steps, decay_factor=0.1, warmup_length=args.warmup,
                logger=logger
            )
        else:
            lr_policy = StepLRScheduler(
                optimizer=optimizer, base_lr=args.lr, steps=[30, 60, 80], decay_factor=0.1, warmup_length=args.warmup,
                logger=logger
            )
    elif args.lr_schedule == "cosine":
        lr_policy = CosineLRScheduler(
            optimizer=optimizer, base_lr=args.lr, warmup_length=args.warmup, epochs=args.epochs, end_lr=args.end_lr,
            logger=logger
        )
    elif args.lr_schedule == "linear":
        lr_policy = LinearLRScheduler(optimizer=optimizer, base_lr=args.lr, warmup_length=args.warmup,
                                      epochs=args.epochs, logger=logger
                                      )
    elif args.lr_schedule == "exponential":
        lr_policy = ExponentialLRScheduler(optimizer=optimizer, base_lr=args.lr, warmup_length=args.warmup,
                                           epochs=args.epochs, logger=logger
                                           )
...
def main(args):
    ...
    if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
        ...
        traced_script_module.save(
            os.path.join(args.output_dir,"export", "model.pt"))
        logger.info(20 * "=")
        logger.info("finish export model...")
        logger.info(20 * "=")
...
if __name__ == "__main__":
    ...
    main(args)
#  python main.py --data_name "dog-vs-cat" --data_path "/home/xky/autodl/torch_pipeline/autotorch/asset/test_dataset/dog-vs-cat/" --output-dir "./checkpoint" --model "resnet18" --epochs 2 --lr 0.01 --batch-size 8